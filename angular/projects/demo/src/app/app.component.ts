...
export class AppComponent implements OnInit {
  ...
  public addChart() {
    // angular detection changes...
    let idSeq = ids++
    let chartIdStr = `chart-${this.chartType}-${idSeq}`;
    this.gridComp?.grid?.addWidget('<div class="grid-stack-item"><div class="grid-stack-item-content" id="' + `${chartIdStr}` +'" ></div></div>', {x:3, y:0, w:3, h:2, content:`${chartIdStr}`, id:String(idSeq)});
    console.log(typeof(this.chartType));
    console.log(this.chartType);
    switch(this.chartType) {
      case 'line': this.createChartLine(chartIdStr); break;
      case 'gauge': this.createChartGauge(chartIdStr); break;
      case 'pie': this.createChartPie(chartIdStr); break;
      case 'sunburst': this.createChartSunburst(chartIdStr); break;
      default:
        this.createChartColumn(chartIdStr);
        break;
    }
  }

  private createChartColumn(chartSelector: string): void {
    // ChartSelector Pattern: chart-column-<series No.>, e.g. 'chart-column-19'
    let date = new Date();
    const data: any[] = [];
    ...
    const chart = Highcharts.chart(chartSelector as any, {
      chart: {
        type: 'column',
      },
      title: {
  ...
  private createChartLine(chartSelector: string): void {
    let date = new Date();
    const data: any[] = [];
  ...
  private createChartGauge(chartSelector: string): void {
    const chart = Highcharts.chart(chartSelector as any, {
      chart: {
        type: 'solidgauge',
      },
  ...
  private createChartPie(chartSelector: string): void {
    let date = new Date();
    const data: any[] = [];
  ...
  private createChartSunburst(chartSelector: string): void {
    const data: any[] = [{
          id: '0.0',
          parent: '',
          name: 'The World'
  ...
  public onChartTypeChange(ent: any){
    console.log(`${this.chartType}`)
  }
}
